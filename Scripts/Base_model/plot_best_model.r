...
setwd("/home/alicia/Uni/TFG/Modelos95")
library(tidyverse)
library(ggpubr)
list.files()
...
plot_best_hd_model <- function(trees_data, species_id, species_name, results_dir = "./Data") {
  ...
  species_dir <- file.path(results_dir, "Modelos95", species_id)
  ...
  # mismo rango (de 0 a max_h) en ambos ejes -> ejes con las mismas dimensiones
  max_h <- max(c(eval_data$h, eval_data$h_pred), na.rm = TRUE)
  ...
  # Save Block 1: Fit and Residuals vs Predictions (1 row, 2 columns) ====
  ...
  block_1_path <- file.path(plot_dir, "bloque_1_ajuste_residuales.png")
  ggplot2::ggsave(
...
# Select parameters (Pinus sylvestris, 21; Pinus nigra, 25; Pinus pinaster, 26; Quercus pyrenaica, 43)
sp <- 26
