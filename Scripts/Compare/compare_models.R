...
setwd("/home/alicia/Uni/TFG/Compare")
...
#' Modelo 1: (especies 21/25 -> elmamoun_2013_M11, 26 -> pearl_1920, 43 -> gompertz_model)
...
compare_hd_models <- function(data_path = "./data/tree_data.csv",
  ...
  # descarta el subinventario senescente JCyL_SEN
  species_data <- species_data %>% dplyr::filter(INVENTORY_ID != "JCyL_SEN")
  
  #  BAL (Basal Area of Larger trees), por si algun modelo
  ...
  # >>> Limpieza: descarta arboles sin las covariables de rodal que usan
  #     los modelos (p.ej. la parcela IFN4_24_1304_A_1 en Quercus 43).
  #     Se hace ANTES del split para que train/test y los 5 modelos se
  #     comparen sobre EXACTAMENTE los mismos arboles.
  stand_covs <- c("Ho", "Do", "dg", "N", "G", "BAL", "ALT")
  stand_covs <- intersect(stand_covs, names(species_data))
  
  n_antes <- nrow(species_data)
  species_data <- species_data %>%
    tidyr::drop_na(dplyr::all_of(c("h", "dbh", stand_covs)))
  n_desc <- n_antes - nrow(species_data)
  cat("Limpieza covariables: ", n_desc, " arboles descartados por NA (",
      round(100 * n_desc / n_antes, 2), "%).\n", sep = "")
  ...
  metrics_5 <- calc_metrics(test_data$h, test_data$pred_h5, "General: Vázquez-Veloso 2025")
  ...
  pred_export <- test_data[, c(id_cols, "dbh", "h",
                               "pred_h1", "pred_h2", "pred_h3", "pred_h4", "pred_h5", "expan")]
  ...
  # plot 1: superimpose curves on the test scatter plot ------ NO RECOMENDADO----
  dbh_seq <- seq(min(test_data$dbh, na.rm = TRUE), max(test_data$dbh, na.rm = TRUE), length.out = 1000)
  
  # >>> Los modelos 2/3/4 dependen de covariables de rodal (Ho, Do, dg, N, G, BAL). 
  # No se pueden evaluar solo con dbh, asi que se fijan a su media en train:
  # cada curva representa el arbol "promedio" de un rodal medio.
  # Covariables de rodal a su media en train; dbh recorre la secuencia.
  # Se toman TODAS las covariables numericas para no dejarnos ninguna que
  # use el fe_combi de algun modelo (p.ej. ALT en Pinus nigra).
  cov_means <- train_data %>%
    dplyr::select(where(is.numeric)) %>%
    dplyr::select(-dplyr::any_of(c("h", "dbh"))) %>%
    dplyr::summarise(dplyr::across(dplyr::everything(), ~ mean(.x, na.rm = TRUE)))
  
  seq_cov <- cbind(
    data.frame(dbh = dbh_seq),
    cov_means[rep(1, length(dbh_seq)), , drop = FALSE]
  )
  rownames(seq_cov) <- NULL
  ...
  pred_long <- test_data %>%
    ...
    dplyr::mutate(
      Modelo = dplyr::case_when(
        ...
        model_id == "pred_h5" ~ "General: Vázque-Veloso 2025"
      )
    )
  
  plot_obs_pred <- ggplot2::ggplot(pred_long, ggplot2::aes(x = h_pred, y = h, color = Modelo)) +
  ...
  # plot 3: residuals vs diameter (heterocedasticidad) 
  residuals_data <- pred_long %>%
    dplyr::mutate(residuals = h_pred - h)
  
  plot_res_dbh <- ggplot2::ggplot(residuals_data, ggplot2::aes(x = dbh, y = residuals, color = Modelo)) +
