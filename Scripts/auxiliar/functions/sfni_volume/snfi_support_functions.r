...
silv_predict_snfi_volume <- function(province, species, dbh = NULL, h = NULL, dnm = NULL, quali ty = "default") {
...
get_snfi_pars <- function(province) {
  ...
  if(!is_num){
    df_provinces <- read.csv("/media/aitor/7FB15A9C1FA8F1DA/trabajo/docencia/TFG_Alicia_Xu_Diez_Gomez/scripts/functions/sfni_volume/ine_regions_provinces.csv") 
    df_provinces <- df_provinces[df_provinces$province_name == province, ]
    province <- df_provinces$province_id  # get province code
  }
    
  # choose parameters file
  pars_file_snfi4 <- paste("/media/aitor/7FB15A9C1FA8F1DA/trabajo/docencia/TFG_Alicia_Xu_Diez_Gomez/scripts/functions/sfni_volume/snfi4/SNFI4_all_volume_coefficients_", province, ".csv", sep = '')  # avv
  pars_file_snfi3 <- paste("/media/aitor/7FB15A9C1FA8F1DA/trabajo/docencia/TFG_Alicia_Xu_Diez_Gomez/scripts/functions/sfni_volume/snfi3/SNFI3_all_volume_coefficients_", province, ".csv", sep = '')  # avv
