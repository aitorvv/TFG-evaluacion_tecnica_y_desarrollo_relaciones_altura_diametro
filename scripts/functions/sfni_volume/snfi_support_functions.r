...
get_snfi_pars <- function(province) {
  ...
  if(!is_num){
    df_provinces <- read.csv("../functions/sfni_volume/ine_regions_provinces.csv") 
    df_provinces <- df_provinces[df_provinces$province_name == province, ]
    province <- df_provinces$province_id  # get province code
  }
    
  # choose parameters file
  pars_file_snfi4 <- paste("../functions/sfni_volume/snfi4/SNFI4_all_volume_coefficients_", province, ".csv", sep = '')  # avv
  pars_file_snfi3 <- paste("../functions/sfni_volume/snfi3/SNFI3_all_volume_coefficients_", province, ".csv", sep = '')  # avv
...
filter_pars <- function(pars, species, quality) {
  ...
  if(nrow(pars) == 0){
    print(paste("No parameters found for the specified species:", species))
